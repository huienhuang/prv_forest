	...
	protected function assignAttributesGroups()
	{
		...
		if (is_array($attributes_groups) && $attributes_groups)
		{
			...
			foreach ($attributes_groups as $k => $row)
			{
				...
				$combinations[$row['id_product_attribute']]['short_desc'] = explode('|', $row['short_desc'], 4);
			...
			foreach ($combinations as $id_product_attribute => $comb)
			{
				...
				$short_desc = [];
				foreach ($comb['attributes_values'] as $id_attribute_group=>$nz)
					$short_desc[] = $groups[$id_attribute_group]['name'] . ': ' . $nz;
				$short_desc = implode(', ', $short_desc);
				$combinations[$id_product_attribute]['desc'] = $short_desc;
